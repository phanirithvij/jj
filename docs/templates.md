...
* `.replace(pattern: StringPattern, replacement: Template, [limit: Integer]) -> String`:
  Replace occurrences of the given `pattern` with the `replacement` string.

  By default, all occurrences are replaced. If `limit` is specified and >=0, at most
  that many occurrences are replaced. If `limit` is <0, all occurrences are replaced.

  Supports capture groups in regex patterns using `$1`, `$2` etc.
* `.first_line() -> String`
